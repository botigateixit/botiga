...
//obtenim el codi de botiga a partir del directori d'execució
$nomdir = getcwd ();
$cbotiga = substr ($nomdir,-1,1);
...
// Inicialitzem el log
$ara = date("Ymd__H:i:s");
$dadeslog = "Dia/Hora execucio Quadre: ".$ara." Botiga numero: ".$cbotiga.PHP_EOL;
...
$sql="SELECT IFNULL(pd.`product_name`,'Total') AS Producte , SUM(pd.`product_quantity`) AS Quantitat , cast(pd.`unit_price_tax_incl` as decimal(10,2)) AS Preu
	...
	WHERE ((p.`current_state` = 2) OR (p.`current_state` = 14)) AND (pc.`id_default_group`= $cbotiga)
...
if ($rproductes->num_rows > 0) {
	...
	// Gravem les dades de la comanda en el fitxer destinat a aquest proveidor en format HTML i a partir de la plantilla
		$fprov = "./".date("Y")."/".$ara."_quadre.html";
		$dadesquadre= file_get_contents("../txtdltrr/plantilles/doc_quadre.htm_");
...
if ($rproductes->num_rows > 0) {
    ...
    // Tanquem amb la plantilla de fi de missatge el fitxer html i gravem
		$dadesquadre.= file_get_contents("../txtdltrr/plantilles/fidoc_quadre.htm_");
